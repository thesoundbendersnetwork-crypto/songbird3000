...
function getAudioFromPublicDatabase(tags: string = "", prompt: string = "") {
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      if (db.tracks && db.tracks.length > 0) {
        const query = `${tags} ${prompt}`.toLowerCase();
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = body.prompt || body.gpt_description_prompt || "A cool song";
    
    let audioUrl = "";
    let lyricsText = "";
    let title = body.title || "Custom AI Song";
    let image = "https://picsum.photos/seed/" + Date.now() + "/400/400";
    let tags = body.tags || "Pop";
    ...
    try {
      ...
      if (audioBase64) {
        ...
        const filename = `lyria_${Date.now()}.${ext}`;
    ...
    if (!audioUrl) {
      const track = getAudioFromPublicDatabase(body.tags || "", prompt);
      audioUrl = track.audioUrl;
      image = track.image || image;
      title = track.title || title;
      tags = track.genre || tags;
    }
      ...
      clips: [
        {
          ...
          title: title,
          major_model_version: body.mv || "v3",
          model_name: body.mv || "chirp-v3-0",
          metadata: {
            tags: tags,
            prompt: prompt,
            gpt_description_prompt: lyricsText || body.gpt_description_prompt || "",
